...
dylint_linting::declare_pre_expansion_lint! {
    /// ### What it does
    ///
    /// Enforces the running-process#965 source boundary: host-platform selection and
    /// native OS APIs may only appear inside the `running-process-platform-internal` leaf
    ...
    /// boundary. One selector in `crates/running-process-platform-internal/src/lib.rs` plus
    ...
    /// in the PR that migrates their code; the all-source companion checker
    /// rejects stale entries without mistaking a host-local module graph for
    /// the cross-host union).
    ///
    /// ### Example
    ///
    /// ```rust,ignore
    ...
    /// `crate::platform::…` facade in running-process-platform-internal.
    pub ENFORCE_PLATFORM_BOUNDARY,
    Deny,
    "confine host-platform selection and native APIs to running-process-platform-internal"
}

/// Host predicate names that select the host platform. These are forbidden
/// outside running-process-platform-internal even when spelled inside `any`/`all`/`not`.
...
/// Native API roots forbidden outside running-process-platform-internal.
...
const SPECIALIZED_ARTIFACT_PREFIXES: &[&str] = &[
    ...
    "crates/running-process-probe/src/crash/",
    "crates/running-process-probe/src/snapshot/mod.rs",
...
/// after a migration.
const DUMP_ENV: &str = "RUNNING_PROCESS_PLATFORM_BOUNDARY_DUMP_DIR";
...
fn classify(path: &str) -> Scope {
    ...
    // or through the repo root (lints/running-process-platform-boundary/ui/…).
    if path.starts_with("ui/") || path.starts_with("lints/running-process-platform-boundary/ui/") {
        return Scope::Ui;
    }
    if path.starts_with("crates/running-process-platform-internal/src/platform_win")
        || path.starts_with("crates/running-process-platform-internal/src/platform_linux")
    ...
    if path.starts_with("crates/running-process-platform-internal/src/") {
        return Scope::Facade;
    }
    ...
    match kind {
        Kind::AttrCfg | Kind::CfgMacro => format!(
            "host-platform cfg selection ({normalized}) is not allowed here; \
             host selection belongs in crates/running-process-platform-internal/src/lib.rs cfg_select!"
        ),
        ...
        Kind::ModuleRef => format!(
            "reference to concrete platform module ({normalized}) is not allowed here; \
             concrete modules stay private to running-process-platform-internal"
        ),
    }
}
...
impl EarlyLintPass for EnforcePlatformBoundary {
    ...
    fn check_crate_post(&mut self, _cx: &EarlyContext<'_>, _krate: &Crate) {
        // The independent all-source checker owns stale-row validation. A
        // compiler invocation sees only the modules selected for its current
        // host/features, so treating an unseen union row as stale here would
        // reject a legitimate Linux/macOS-only ledger entry on Windows.
    }
}
