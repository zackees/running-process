...
- **`running-process`** (`crates/running-process/`): the crate consumers depend on. Feature-gated subsystems:
  ...
  - **`pty`** — PTY-backed process APIs (`portable-pty` on Unix, ConPTY on Windows), including terminal graphics capability reporting.
  - **`terminal-graphics`** — terminal capability serialization; composed by `pty` and `client`.
  - **`daemon-trampoline`** (default) — standalone JSON-sidecar launcher binary; its serialization does not enter a process-only graph.
  - **`process-inspection`** — host inspection via `sysinfo`; composed by `daemon`, `originator-scan`, and `probe`.
  - **`client`** (default) — generated protocol types via `running-process-protocol` + sync IPC client (`src/client/`). Adds prost, interprocess, dirs, and local client-config serialization.
  ...
  - Binaries in `src/bin/`: `runpm` (requires `client`), `daemon` (requires `daemon`), `trampoline` (requires `daemon-trampoline`), `running-process-broker-v1` / `running-process-broker-v2` (broker scaffold for #483/#488/#532), `running-process-cleanup`.
  - Broker and daemon schemas live in `running-process-protocol`; the root crate re-exports generated types only through `client` (#1144).
