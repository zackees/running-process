...
def create_sketch_path_formatter(example: str) -> OutputFormatter:
    """Create a formatter that maps lib/FastLED paths to src/ and sketch paths to examples/{example}/ with timestamps.
    ...
    display_example_str = example.replace("\\", "/") if "/" in example or "\\" in example else f"examples/{example}"
