...
- `uv run build.py` defaults to a dev editable rebuild via `maturin develop --uv --profile dev`
...
- `zccache` belongs to the repo's `dev` dependency group and should only be active in `build.py` dev mode, never in release builds
